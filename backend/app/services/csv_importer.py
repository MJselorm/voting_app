...
async def preview_upload(content: bytes, filename: str, db: AsyncSession) -> Preview:
    ...
    existing_by_id = {s.student_id.strip().upper(): s for s in (await db.execute(select(Student))).scalars() if s.student_id}
    emails = {s.email.lower(): s.student_id for s in existing_by_id.values() if s.email}
    ...
    for record in records:
        ...
        sid, email = record.get("student_id", "").strip(), record.get("email", "").strip()
        if sid and sid.upper() in seen_ids: reasons.append("Duplicate Student ID inside uploaded file.")
        if email and email.lower() in seen_emails: reasons.append("Duplicate email inside uploaded file.")
        if sid: seen_ids.add(sid.upper())
        if email: seen_emails.add(email.lower())
        if email and email.lower() in emails and emails[email.lower()].strip().upper() != sid.upper(): reasons.append("Email belongs to a different existing student.")
        if reasons:
        ...
        elif sid and sid.upper() in existing_by_id:
...
async def apply_preview(records: list[dict], behavior: str, db: AsyncSession) -> tuple[int, int, int]:
    ...
    for rec in records:
        ...
        sid = rec["student_id"].strip()
        current = (await db.execute(select(Student).where(func.lower(Student.student_id) == sid.lower()))).scalar_one_or_none()
        ...
        if current:
            ...
            db.add(Student(student_id=sid, full_name=rec["full_name"], email=rec["email"], department=rec["department"], level=rec["level"], class_=rec["class"], status=rec["status"])); added += 1
    await db.flush()
